...
void port_system_power_stop()
{
  ...
  MODIFY_REG(PWR->CR, (PWR_CR_PDDS | PWR_CR_LPDS), PWR_CR_LPDS);

  /* Set SLEEPDEEP bit of Cortex System Control Register */
  SCB->SCR |= ((uint32_t)SCB_SCR_SLEEPDEEP_Msk);

  /* Select Stop mode entry : Request Wait For Interrupt */
  __WFI();
...
{
  /* Select the regulator state in sleep mode: Set PDDS and LPDS bits according to PWR_Regulator value */
  MODIFY_REG(PWR->CR, (PWR_CR_PDDS | PWR_CR_LPDS), PWR_CR_LPDS);

  /* Reset SLEEPDEEP bit of Cortex System Control Register */
  SCB->SCR &= ~((uint32_t)SCB_SCR_SLEEPDEEP_Msk);

  /* Select sleep mode entry : Request Wait For Interrupt */
  __WFI();
}

void port_system_systick_resume()
{
  // TO-DO alumnos: Implement the function
...
void port_system_sleep()
{
  ...
  port_system_power_sleep(); // If you need to keep PWM ON, or USART, or...
}